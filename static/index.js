...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score <= 5) return 'うるせ～な';
            if (score <= 8) return 'おいおい';
            if (score <= 10)  return '吾輩の勝利じゃ';
            if (score <= 15) return '震撼しやげれ、愚民ども！';
            return '俺の出番が';
        } else {
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "朔间小零";
        if (cookie("keyboard")) {
